	...
	#define V_COMMIT_HASH "6749979"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "6749979"
#endif
...
typedef struct Option_Array_rune Option_Array_rune;
...
#include <termios.h>
#else
#error VERROR_MESSAGE Header file <termios.h>, needed for module `readline` was not found. Please install the corresponding development headers.
#endif
...
#include <sys/ioctl.h>
#else
#error VERROR_MESSAGE Header file <sys/ioctl.h>, needed for module `readline` was not found. Please install the corresponding development headers.
#endif
...
typedef enum {
	...
	readline__Action__move_cursor_left, // +6
	readline__Action__move_cursor_right, // +7
	readline__Action__move_cursor_begining, // +8
	readline__Action__move_cursor_end, // +9
	readline__Action__move_cursor_word_left, // +10
	readline__Action__move_cursor_word_right, // +11
	readline__Action__history_previous, // +12
	readline__Action__history_next, // +13
	readline__Action__overwrite, // +14
	readline__Action__clear_screen, // +15
	readline__Action__suspend, // +16
} readline__Action;
...
struct Option_Array_rune {
	byte state;
	IError err;
	byte data[sizeof(Array_rune)];
};
...
#define _const_readline__cclen 10
...
Option_Array_rune readline__Readline_read_line_utf8(readline__Readline* r, string prompt);
Option_string readline__Readline_read_line(readline__Readline* r, string prompt);
Option_Array_rune readline__read_line_utf8(string prompt);
...
VV_LOCAL_SYMBOL readline__Action readline__Readline_analyse_extended_control_no_eat(readline__Readline* r, byte last_c);
...
VV_LOCAL_SYMBOL void readline__Readline_move_cursor_begining(readline__Readline* r);
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	...
	if (sidx == _IError_os__ErrFileNotOpened_index) return 65665;
	if (sidx == _IError_os__ErrSizeOfTypeIs0_index) return 65666;
	if (sidx == _IError_os__ErrExecutableNotFound_index) return 65678;
	return 29;
}
...
Option_void os__cp(string src, string dst) {
	...
	#else
	{
		...
		int count = 0;
		for (;;) {
			count = read(fp_from, &buf[0], sizeof(Array_fixed_byte_1024));
			if (count == 0) {
				break;
			}
...
VV_LOCAL_SYMBOL int os__vpclose(voidptr f) {
	...
	#else
	{
		multi_return_int_bool mr_8931 = os__posix_wait4_to_exit_status(pclose(f));
		int ret = mr_8931.arg0;
		int _t1 = ret;
		return _t1;
	}
	#endif
...
int os__system(string cmd) {
	...
	#if !defined(_WIN32)
	{
		multi_return_int_bool mr_9940 = os__posix_wait4_to_exit_status(ret);
		int pret = mr_9940.arg0;
		bool is_signaled = mr_9940.arg1;
		if (is_signaled) {
...
string os__get_raw_line(void) {
	...
	#else
	{
		...
		string _t1 = tos(((byte*)(buf)), (nr_chars < 0 ? (0) : (nr_chars)));
		return _t1;
	}
	#endif
	return (string){.str=(byteptr)"", .is_lit=1};
...
Array_byte os__get_raw_stdin(void) {
	...
	#else
	{
		...
		char* buf = ((char*)(0));
		int nr_chars = getline(&buf, &max, stdin);
		Array_byte _t1 = (array){.element_size = 1,.data = ((voidptr)(buf)),.offset = 0,.len = (nr_chars < 0 ? (0) : (nr_chars)),.cap = ((int)(max)),.flags = 0,};
		return _t1;
	}
	#endif
...
Option_Array_string os__glob(Array_string patterns) {
 ...
 ;
	...
	qsort(matches.data, matches.len, matches.element_size, (int (*)(const void *, const void *))&compare_string);
...
VV_LOCAL_SYMBOL Array_string os__glob_match(string dir, string pattern, string next_pattern, Array_string* matches) {
	...
	for (int _t7 = 0; _t7 < files.len; ++_t7) {
		...
		if (string_contains(file, _const_os__path_separator)) {
			...
			_t8 = (*(string*)/*ee elem_typ */array_get(pathwalk, pathwalk.len - 1));
		} else {
...
strings__Builder res;
	string pcmd = (string_contains(cmd, _SLIT("2>")) ? (cmd) : ( str_intp(2, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = cmd}}, {_SLIT(" 2>&1"), 0, { .d_c = 0 }}}))));
...
string os__Process_stdout_read(os__Process* p) {
	...
	#else
	{
		multi_return_string_int mr_3530 = os__fd_read(p->stdio_fd[1], 4096);
		string s = mr_3530.arg0;
		string _t1 = s;
		return _t1;
	}
	#endif
...
string os__Process_stderr_read(os__Process* p) {
	...
	#else
	{
		multi_return_string_int mr_3748 = os__fd_read(p->stdio_fd[2], 4096);
		string s = mr_3748.arg0;
		string _t1 = s;
		return _t1;
	}
	#endif
...
VV_LOCAL_SYMBOL void os__Process_unix_wait(os__Process* p) {
	...
	multi_return_int_bool mr_2289 = os__posix_wait4_to_exit_status(cstatus);
	int pret = mr_2289.arg0;
	bool is_signaled = mr_2289.arg1;
	if (is_signaled) {
		p->status = os__ProcessState__aborted;
...
VV_LOCAL_SYMBOL bool os__Process_unix_is_alive(os__Process* p) {
	...
	multi_return_int_bool mr_2722 = os__posix_wait4_to_exit_status(cstatus);
	int pret = mr_2722.arg0;
	bool is_signaled = mr_2722.arg1;
	if (is_signaled) {
		p->status = os__ProcessState__aborted;
		p->err =  str_intp(3, _MOV((StrIntpData[]){{_SLIT("Terminated by signal "), 0x4fe27, {.d_i32 = ret}}, {_SLIT(" ("), 0xfe10, {.d_s = os__sigint_to_signal_name(pret)}}, {_SLIT(")"), 0, { .d_c = 0 }}}));
	} else {
...
struct termios old_state;
	if (os__is_atty(1) <= 0 || string__eq(os__getenv(_SLIT("TERM")), _SLIT("dumb"))) {
		Option_term__Coord _t1;
		opt_ok(&(term__Coord[]) { (term__Coord){.x = 0,.y = 0,} }, (Option*)(&_t1), sizeof(term__Coord));
		return _t1;
	}
	...
	if (tcgetattr(0, &state) != 0) {
		Option_term__Coord _t3 = (Option_term__Coord){ .state=2, .err=os__last_error(), .data={EMPTY_STRUCT_INITIALIZATION} };
		// Defer begin
	...
	for (;;) {
		int w = getchar();
		if (w < 0) {
			Option_term__Coord _t4 = (Option_term__Coord){ .state=2, .err=error_with_code(_SLIT("Failed to read from stdin"), 888), .data={EMPTY_STRUCT_INITIALIZATION} };
			// Defer begin
			if (term__get_cursor_position_defer_0) {
				tcsetattr(0, TCSANOW, &old_state);
			}
	...
	Option_term__Coord _t5;
	opt_ok(&(term__Coord[]) { (term__Coord){.x = x,.y = y,} }, (Option*)(&_t5), sizeof(term__Coord));
	// Defer begin
	if (term__get_cursor_position_defer_0) {
		tcsetattr(0, TCSANOW, &old_state);
	}
...
void readline__Readline_enable_raw_mode(readline__Readline* r) {
	if (tcgetattr(0, ((struct termios*)(&r->orig_termios))) != 0) {
		r->is_tty = false;
		r->is_raw = false;
		return;
	}
	struct termios raw = (struct termios){.c_iflag = 0,.c_oflag = 0,.c_cflag = 0,.c_lflag = 0,.c_line = 0,.c_cc = {0},};
	vmemcpy(&raw, &r->orig_termios, ((int)(sizeof(struct termios))));
	raw.c_iflag &= ~(((((BRKINT | ICRNL) | INPCK) | ISTRIP) | IXON));
	raw.c_cflag |= CS8;
	raw.c_lflag &= ~((((ECHO | ICANON) | IEXTEN) | ISIG));
	raw.c_cc[v_fixed_index(VMIN, 10)] = ((byte)(1));
	raw.c_cc[v_fixed_index(VTIME, 10)] = ((byte)(0));
...
void readline__Readline_disable_raw_mode(readline__Readline* r) {
	if (r->is_raw) {
		tcsetattr(0, TCSADRAIN, ((struct termios*)(&r->orig_termios)));
		r->is_raw = false;
	}
}

int readline__Readline_read_char(readline__Readline* r) {
	int _t1 = utf8_getchar();
	return _t1;
}

Option_Array_rune readline__Readline_read_line_utf8(readline__Readline* r, string prompt) {
	r->current = __new_array_with_default(0, 0, sizeof(rune), 0);
	r->cursor = 0;
	r->prompt = prompt;
	r->search_index = 0;
	r->prompt_offset = readline__get_prompt_offset(prompt);
	if (r->previous_lines.len <= 1) {
		array_push((array*)&r->previous_lines, _MOV((Array_rune[]){ __new_array_with_default(0, 0, sizeof(rune), 0) }));
		array_push((array*)&r->previous_lines, _MOV((Array_rune[]){ __new_array_with_default(0, 0, sizeof(rune), 0) }));
	} else {
		(*(Array_rune*)/*ee elem_typ */array_get(r->previous_lines, 0)) = __new_array_with_default(0, 0, sizeof(rune), 0);
	}
	if (!r->is_raw) {
	...
	if (r->current.len == 0) {
		return (Option_Array_rune){ .state=2, .err=_v_error(_SLIT("empty line")), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	Option_Array_rune _t4;
	opt_ok(&(Array_rune[]) { r->current }, (Option*)(&_t4), sizeof(Array_rune));
	return _t4;
}

Option_string readline__Readline_read_line(readline__Readline* r, string prompt) {
	Option_Array_rune _t1 = readline__Readline_read_line_utf8(r, prompt);
	if (_t1.state != 0) { /*or block*/ 
		Option_string _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
	...
	Option_string _t3;
	opt_ok(&(string[]) { Array_rune_string(s) }, (Option*)(&_t3), sizeof(string));
	return _t3;
}

Option_Array_rune readline__read_line_utf8(string prompt) {
	readline__Readline r = (readline__Readline){.is_raw = 0,.orig_termios = (readline__Termios){.c_iflag = 0,.c_oflag = 0,.c_cflag = 0,.c_lflag = 0,.c_line = 0,},.current = __new_array(0, 0, sizeof(rune)),.cursor = 0,.overwrite = 0,.cursor_row_offset = 0,.prompt = (string){.str=(byteptr)"", .is_lit=1},.prompt_offset = 0,.previous_lines = __new_array(0, 0, sizeof(Array_rune)),.search_index = 0,.is_tty = 0,};
	Option_Array_rune _t1 = readline__Readline_read_line_utf8(&r, prompt);
	if (_t1.state != 0) { /*or block*/ 
		Option_Array_rune _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
	}
	
 	Array_rune s =  (*(Array_rune*)_t1.data);
	Option_Array_rune _t3;
	opt_ok(&(Array_rune[]) { s }, (Option*)(&_t3), sizeof(Array_rune));
	return _t3;
}

Option_string readline__read_line(string prompt) {
	readline__Readline r = (readline__Readline){.is_raw = 0,.orig_termios = (readline__Termios){.c_iflag = 0,.c_oflag = 0,.c_cflag = 0,.c_lflag = 0,.c_line = 0,},.current = __new_array(0, 0, sizeof(rune)),.cursor = 0,.overwrite = 0,.cursor_row_offset = 0,.prompt = (string){.str=(byteptr)"", .is_lit=1},.prompt_offset = 0,.previous_lines = __new_array(0, 0, sizeof(Array_rune)),.search_index = 0,.is_tty = 0,};
	Option_string _t1 = readline__Readline_read_line(&r, prompt);
	if (_t1.state != 0) { /*or block*/ 
		Option_string _t2;
		memcpy(&_t2, &_t1, sizeof(Option));
		return _t2;
	...
	Option_string _t3;
	opt_ok(&(string[]) { s }, (Option*)(&_t3), sizeof(string));
	return _t3;
}
...
VV_LOCAL_SYMBOL readline__Action readline__Readline_analyse(readline__Readline* r, int c) {
	...
	if (_t1 == ('\0') || _t1 == (0x3) || _t1 == (0x4) || _t1 == (255)) {
		readline__Action _t2 = readline__Action__eof;
		return _t2;
	}
	else if (_t1 == ('\n') || _t1 == ('\r')) {
		readline__Action _t3 = readline__Action__commit_line;
		return _t3;
	}
	else if (_t1 == ('\f')) {
		readline__Action _t4 = readline__Action__clear_screen;
		return _t4;
	}
	else if (_t1 == ('\b') || _t1 == (127)) {
		readline__Action _t5 = readline__Action__delete_left;
		return _t5;
	}
	else if (_t1 == (27)) {
		readline__Action _t6 = readline__Readline_analyse_control(r);
		return _t6;
	}
	else if (_t1 == (1)) {
		readline__Action _t7 = readline__Action__move_cursor_begining;
		return _t7;
	}
	else if (_t1 == (5)) {
		readline__Action _t8 = readline__Action__move_cursor_end;
		return _t8;
	}
	else if (_t1 == (26)) {
		readline__Action _t9 = readline__Action__suspend;
		return _t9;
	}
	else {
		if (c >= ' ') {
			readline__Action _t10 = readline__Action__insert_character;
			return _t10;
		}
		readline__Action _t11 = readline__Action__nothing;
		return _t11;
	};
	return 0;
}
...
VV_LOCAL_SYMBOL readline__Action readline__Readline_analyse_control(readline__Readline* r) {
	...
	if (_t1 == ('[')) {
		...
		byte _t2 = ((byte)(sequence));
		
		if (_t2 == ('C')) {
			readline__Action _t3 = readline__Action__move_cursor_right;
			return _t3;
		}
		else if (_t2 == ('D')) {
			readline__Action _t4 = readline__Action__move_cursor_left;
			return _t4;
		}
		else if (_t2 == ('B')) {
			readline__Action _t5 = readline__Action__history_next;
			return _t5;
		}
		else if (_t2 == ('A')) {
			readline__Action _t6 = readline__Action__history_previous;
			return _t6;
		}
		else if (_t2 == ('1')) {
			readline__Action _t7 = readline__Readline_analyse_extended_control(r);
			return _t7;
		}
		else if (_t2 == ('2') || _t2 == ('3')) {
			readline__Action _t8 = readline__Readline_analyse_extended_control_no_eat(r, ((byte)(sequence)));
			return _t8;
		}
		else {
		};
	...
	readline__Action _t9 = readline__Action__nothing;
	return _t9;
}
...
VV_LOCAL_SYMBOL readline__Action readline__Readline_analyse_extended_control(readline__Readline* r) {
	...
	if (_t1 == ('5')) {
		...
		byte _t2 = ((byte)(direction));
		
		if (_t2 == ('C')) {
			readline__Action _t3 = readline__Action__move_cursor_word_right;
			return _t3;
		}
		else if (_t2 == ('D')) {
			readline__Action _t4 = readline__Action__move_cursor_word_left;
			return _t4;
		}
		else {
		};
...
VV_LOCAL_SYMBOL readline__Action readline__Readline_analyse_extended_control_no_eat(readline__Readline* r, byte last_c) {
	...
	if (_t1 == ('~')) {
		...
		if (last_c == ('3')) {
			readline__Action _t2 = readline__Action__delete_right;
			return _t2;
		}
		else if (last_c == ('2')) {
			readline__Action _t3 = readline__Action__overwrite;
			return _t3;
		}
		else {
		};
	...
	readline__Action _t4 = readline__Action__nothing;
	return _t4;
}
...
VV_LOCAL_SYMBOL bool readline__Readline_execute(readline__Readline* r, readline__Action a, int c) {
	switch (a) {
		...
		case readline__Action__move_cursor_begining: 
			{
...
VV_LOCAL_SYMBOL void readline__shift_cursor(int xpos, int yoffset) {
	...
	print( str_intp(2, _MOV((StrIntpData[]){{_SLIT("\x1b["), 0xfe07, {.d_i32 = xpos + 1}}, {_SLIT("G"), 0, { .d_c = 0 }}})));
}

VV_LOCAL_SYMBOL Array_int readline__calculate_screen_position(int x_in, int y_in, int screen_columns, int char_count, Array_int inp) {
	Array_int out = array_clone_to_depth(&inp, 0);
	int x = x_in;
	int y = y_in;
	array_set(&out, 0, &(int[]) { x });
	array_set(&out, 1, &(int[]) { y });
	for (int chars_remaining = char_count; chars_remaining > 0; ) {
		int chars_this_row = ((x + chars_remaining) < screen_columns ? (chars_remaining) : (screen_columns - x));
	...
	if ((*(int*)/*ee elem_typ */array_get(out, 0)) == screen_columns) {
		array_set(&out, 0, &(int[]) { 0 });
		(*(int*)/*ee elem_typ */array_get(out, 1))++;
	}
	Array_int _t1 = out;
	return _t1;
}
...
VV_LOCAL_SYMBOL int readline__get_prompt_offset(string prompt) {
	...
	int _t1 = prompt.len - len;
	return _t1;
}

VV_LOCAL_SYMBOL void readline__Readline_refresh_line(readline__Readline* r) {
	Array_int end_of_input = new_array_from_c_array(2, 2, sizeof(int), _MOV((int[2]){0, 0}));
	end_of_input = readline__calculate_screen_position(r->prompt.len, 0, readline__get_screen_columns(), r->current.len, end_of_input);
	Array_rune _t1 = __new_array(0, 0, sizeof(rune));
	Array_rune _t1_orig = r->current;
	int _t1_len = _t1_orig.len;
	_t1 = __new_array(0, _t1_len, sizeof(rune));
	...
	Array_int cursor_pos = new_array_from_c_array(2, 2, sizeof(int), _MOV((int[2]){0, 0}));
	...
	if ((*(int*)/*ee elem_typ */array_get(end_of_input, 0)) == 0 && (*(int*)/*ee elem_typ */array_get(end_of_input, 1)) > 0) {
		print(_SLIT("\n"));
	}
	readline__shift_cursor((*(int*)/*ee elem_typ */array_get(cursor_pos, 0)) - r->prompt_offset, -((*(int*)/*ee elem_typ */array_get(end_of_input, 1)) - (*(int*)/*ee elem_typ */array_get(cursor_pos, 1))));
	r->cursor_row_offset = (*(int*)/*ee elem_typ */array_get(cursor_pos, 1));
}
...
VV_LOCAL_SYMBOL void readline__Readline_insert_character(readline__Readline* r, int c) {
	if (!r->overwrite || r->cursor == r->current.len) {
		array_insert(&r->current, r->cursor, &(rune[]){c});
	} else {
		(*(rune*)/*ee elem_typ */array_get(r->current, r->cursor)) = ((rune)(c));
	}
	r->cursor++;
	if (r->is_tty) {
		readline__Readline_refresh_line(r);
...
VV_LOCAL_SYMBOL bool readline__Readline_commit_line(readline__Readline* r) {
	array_insert(&r->previous_lines, 1, &(Array_rune[]){r->current});
	array_push((array*)&r->current, _MOV((rune[]){ '\n' }));
	r->cursor = r->current.len;
	if (r->is_tty) {
		readline__Readline_refresh_line(r);
		println(_SLIT(""));
...
VV_LOCAL_SYMBOL void readline__Readline_move_cursor_word_left(readline__Readline* r) {
	if (r->cursor > 0) {
		for (; r->cursor > 0 && readline__Readline_is_break_character(r, rune_str((*(rune*)/*ee elem_typ */array_get(r->current, r->cursor - 1)))); r->cursor--) {
		}
		for (; r->cursor > 0 && !readline__Readline_is_break_character(r, rune_str((*(rune*)/*ee elem_typ */array_get(r->current, r->cursor - 1)))); r->cursor--) {
		}
		readline__Readline_refresh_line(r);
...
VV_LOCAL_SYMBOL void readline__Readline_move_cursor_word_right(readline__Readline* r) {
	if (r->cursor < r->current.len) {
		for (; r->cursor < r->current.len && readline__Readline_is_break_character(r, rune_str((*(rune*)/*ee elem_typ */array_get(r->current, r->cursor)))); r->cursor++) {
		}
		for (; r->cursor < r->current.len && !readline__Readline_is_break_character(r, rune_str((*(rune*)/*ee elem_typ */array_get(r->current, r->cursor)))); r->cursor++) {
		}
		readline__Readline_refresh_line(r);
...
VV_LOCAL_SYMBOL void readline__Readline_history_previous(readline__Readline* r) {
	...
	if (r->search_index == 0) {
		(*(Array_rune*)/*ee elem_typ */array_get(r->previous_lines, 0)) = r->current;
	}
	r->search_index++;
	r->current = (*(Array_rune*)/*ee elem_typ */array_get(r->previous_lines, r->search_index));
	r->cursor = r->current.len;
	readline__Readline_refresh_line(r);
}
...
VV_LOCAL_SYMBOL void main__main(void) {
	readline__Readline reader = (readline__Readline){.is_raw = 0,.orig_termios = (readline__Termios){.c_iflag = 0,.c_oflag = 0,.c_cflag = 0,.c_lflag = 0,.c_line = 0,},.current = __new_array(0, 0, sizeof(rune)),.cursor = 0,.overwrite = 0,.cursor_row_offset = 0,.prompt = (string){.str=(byteptr)"", .is_lit=1},.prompt_offset = 0,.previous_lines = __new_array(0, 0, sizeof(Array_rune)),.search_index = 0,.is_tty = 0,};
	Option_string _t1 = readline__Readline_read_line(&reader, _SLIT("type expresion : "));
	if (_t1.state != 0) { /*or block*/ 
		panic_optional_not_set(*_t1.err.msg);
	}
	
 	main__execv_blod( (*(string*)_t1.data));
}
...
VV_LOCAL_SYMBOL void main__execv_blod(string code) {
	...
	for (int _t1 = 0; _t1 < code.len; ++_t1) {
		byte _v_char = code.str[_t1];
		string cur = byte_ascii_str(_v_char);
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	{ // Initializations for module readline :
	}
