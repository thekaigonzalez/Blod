...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	if (sidx == _IError_None___index) return 65628;
	if (sidx == _IError_Error_index) return 91;
	if (sidx == _IError_os__ErrFileNotOpened_index) return 65668;
	if (sidx == _IError_os__ErrSizeOfTypeIs0_index) return 65669;
	if (sidx == _IError_os__ErrExecutableNotFound_index) return 65682;
	return 29;
}
...
Option_Array_rune readline__Readline_read_line_utf8(readline__Readline* r, string prompt) {
	...
	print(r->prompt);
	r->current = string_runes(os__get_raw_line());
	(*(Array_rune*)/*ee elem_typ */array_get(r->previous_lines, 0)) = __new_array_with_default(0, 0, sizeof(rune), 0);
	r->search_index = 0;
	if (r->current.len == 0) {
...
Option_Array_rune readline__read_line_utf8(string prompt) {
	readline__Readline r = (readline__Readline){.is_raw = 0,.current = __new_array(0, 0, sizeof(rune)),.cursor = 0,.overwrite = 0,.cursor_row_offset = 0,.prompt = (string){.str=(byteptr)"", .is_lit=1},.prompt_offset = 0,.previous_lines = __new_array(0, 0, sizeof(Array_rune)),.search_index = 0,.is_tty = 0,};
	Option_Array_rune _t1 = readline__Readline_read_line_utf8(&r, prompt);
	if (_t1.state != 0) { /*or block*/ 
...
VV_LOCAL_SYMBOL void main__main(void) {
	readline__Readline reader = (readline__Readline){.is_raw = 0,.current = __new_array(0, 0, sizeof(rune)),.cursor = 0,.overwrite = 0,.cursor_row_offset = 0,.prompt = (string){.str=(byteptr)"", .is_lit=1},.prompt_offset = 0,.previous_lines = __new_array(0, 0, sizeof(Array_rune)),.search_index = 0,.is_tty = 0,};
